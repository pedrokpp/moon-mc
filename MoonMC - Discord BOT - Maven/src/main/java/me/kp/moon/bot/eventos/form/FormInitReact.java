        ...
        if (event.getReaction().getReactionEmote().getEmoji().equals("📝")) { // formulário de trial
            ...
            if (FormUtils.hasThread(event.getUserId())) {
                try {
                    event.getUser().openPrivateChannel().queue(dm ->
                            dm.sendMessage("Você já possui um formulário em aberto. Por favor aguarde o encerramento do atual.").queue(
                                    message -> message.delete().queueAfter(3, TimeUnit.SECONDS),
                                    error -> event.getChannel().sendMessage(event.getUser().getAsMention() + " você já possui um formulário em aberto.").queue(
                                            message -> message.delete().queueAfter(3, TimeUnit.SECONDS)
                                    )));
                } catch (Exception exception) {
                    event.getChannel().sendMessage(event.getUser().getAsMention() + " você precisa habilitar suas mensagens privadas para isso.").queue(
                            message -> message.delete().queueAfter(3, TimeUnit.SECONDS));
                }
                return;
            }
            try {
                event.getUser().openPrivateChannel().queue(dm -> dm.sendMessage(new EmbedBuilder()
                        ...
                        .build()).queue(null, fail -> event.getChannel().sendMessage(event.getUser().getAsMention() + " você precisa habilitar suas mensagens privadas para isso.").queue(
                        message -> message.delete().queueAfter(3, TimeUnit.SECONDS))));
            } catch (Exception exception) {
                event.getChannel().sendMessage(event.getUser().getAsMention() + " você precisa habilitar suas mensagens privadas para isso.").queue(
                        message -> message.delete().queueAfter(3, TimeUnit.SECONDS));
