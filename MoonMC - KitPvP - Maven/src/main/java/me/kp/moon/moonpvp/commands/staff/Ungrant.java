...
public class Ungrant implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("ungrant")) {
            ...
            webhook.setContent("O player **" + player.getName() + "** deu unset na permissão ``" + args[1] + "`` para **" + target.getName() + "**.");
            try {
                webhook.execute();
            } catch (IOException e) {
            ...
            player.sendMessage("§aPermissão §e" + args[1] + " §arevogada do player §e" + target.getName());
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "lp user " + target.getName() + " permission unset " + args[1]);
        }
        return false;
    }
